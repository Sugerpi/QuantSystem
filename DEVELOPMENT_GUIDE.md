...
│
├── presentation/
│   └── dashboard.py             #   Streamlit，唯讀，只吃 runs/ 下的 artifacts
│
├── tests/                       # §8
...
- `presentation` **只讀檔案**（parquet + JSON），永不 import 引擎模組、永不觸發計算。舊系統 dashboard 依賴 `store_details=True` pickle 的隱性耦合就此斷根。
...
v1 實作 `YFinanceAdapter` 與 `FredAdapter`。未來的 `EODHDAdapter` 或個股 point-in-time 供應商只需實作同一介面。**引擎的任何模組不得 import yfinance**——只認識 Provider 介面與快照格式。
...
class Decision:
    ...
    diagnostics: dict                  # 動量分數、σ̂、E(t) 等，供 dashboard
...
### Phase 1 — 資料層（~1 週）
Provider 介面、yfinance/FRED adapter、快照建立與 hash、NYSE 日曆、驗證規則。
**AC**：`snapshot create` 產出完整快照；總報酬驗證通過；同日重建兩次快照 hash 相同；test_data/ 全綠。
...
### Phase 7 — Dashboard 與報告（~1 週）
Streamlit 唯讀 dashboard（NAV、權重、曝險、動量排名、GARCH 軌跡、消融比較）；最終研究報告。
...
- presentation/ 只讀 runs/ 下的檔案，不 import 引擎。
...
*v1.0 — 2026-07-08。設計決策經兩輪討論定案：美股 ETF 選單自動選標的、動量選擇 + GARCH/DCC 定倉位、免費資料 + 預留付費層、不做新聞層。*
