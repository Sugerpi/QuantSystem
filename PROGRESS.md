...
- 2026-08-02：**Phase 7b Run Lab 完成，AC②③ 達成**——網頁提交 YAML config → 跑回測 → 監看進度 → 拿結果，全程不碰終端機。設計/計畫見 `docs/superpowers/specs/2026-08-01-phase7b-run-lab-design.md` + `plans/2026-08-01-phase7b-run-lab-plan{1,2}-*.md`。**計畫 1 引擎端狀態管線**（3 task）：`experiments/jobstatus.py`（原子 merge 寫 status.json）、`run_experiment` 加 `on_progress`（每策略回呼，預設 None 向後相容）、`runner._cli` 加 `--status-file`/`--job-id`/`--validate-only` + 逐階段狀態（validating→loading_snapshot→running i/N→done/error）。引擎預設行為逐位元不變。**計畫 2 presentation**（4 task）：`presentation/jobs.py`（job store + 排程器，**不 import 引擎**、只 subprocess 呼叫 CLI；submit/list/validate/`_next_action` 純決策/detached spawn 立刻寫 running+pid 關 race/`reconcile_and_advance` psutil pid-liveness）、Run Lab 頁面（YAML 編輯器/驗證/提交/job 清單 HTMX 輪詢/狀態徽章）、app lifespan 背景心跳執行緒推排隊（一次一個 + 排隊）、退役最後的 stubs、+psutil/+python-multipart。狀態由引擎子行程權威擁有、UI 只讀（原子寫）。以 subagent-driven TDD 執行（每 task 實作+spec+品質 review），**過程抓修 3 個真 bug**：job_id 微秒碰撞會靜默覆蓋 job（加 uuid 後綴）、`_spawn` stdout.log 檔柄靠 GC（改 context manager）、`jobs.validate` subprocess Windows cp950 UnicodeDecodeError（加 encoding=utf-8）。**AC③**（崩潰復原）：detached 子行程存活 + status.json + pid-liveness reconcile（running+死 pid→failed→spawn next），CI 安全測試守。**AC② 端到端本機實跑到 done**（TestClient 提交 canonical_ewma → 排程 → detached 引擎子行程 ~170s → `runs/2026-08-02_0852_e2e/` 產物齊，requires_snapshot 本機閘門）。全套非快照 pytest 綠（presentation 88）、ruff 乾淨、§2.2 架構守護涵蓋 jobs.py/run_lab.py。**7a+7b 完成；唯讀 9 頁 + Run Lab 齊備。剩 7c 研究報告。分支 `feature/phase7a-dashboard` 未併回 main。**
- 2026-08-14：**曝險更新帶 log(相對)模式完成並 `--no-ff` 併回 main**（分支 `feature/exposure-band-log-mode`，merge `38585e6`）。動機：診斷發現曝險檢查日帶突破率 16.1% 且**分布不均**（平靜年 6-8%、動盪年 29-31%），根因非 σ̂_p 估計 bug（讀碼＋實測雙證：refit/filter 的 ×100/÷100² 縮放一致、selection→exposure_check 交界 σ̂_p 變動最小＝無估計器斷層），而是 `E=σ*/σ̂_p` 的**凸性**（`dE/dσ̂_p=−E/σ̂_p` 在 σ̂_p≈σ* 放大）＋**絕對**帶。修法：`target_exposure` 加 `band_mode`（`absolute`|`log`），log 判 `|ln(clipped)−ln(e_current)|>band`（相對容忍度、與波動水準無關），純數學等價 `|Δσ̂_p/σ̂_p|`。以 config 選項落地（`risk.exposure_band_mode`，schema 加 log 需 `exposure_min>0` 守護＋函數內防呆），**default 維持 absolute → 引擎預設路徑逐位元不變（INV-6）**。附 `compute_metrics.annualized_vol`（§6.4）與換手率-追蹤誤差前沿消融工具 `experiments/exposure_band_frontier.py`（`run_frontier` 跑 absolute/log 兩趟帶寬掃描、Pareto 前沿＋弱支配判定）。以 subagent-driven TDD 執行 8 個 task，每項經 spec + code-quality 兩段式 review + 一次整體 cohesion review（判 ready to merge、無 Critical/Important）。合併結果不變量 37/37 + 所有相關測試綠。**AC5（翻不翻 default）暫不翻**：首份真實消融（GARCH+全歷史、網格 absolute{0.06-0.16}/log{0.10-0.25}）機械判定 full:log 支配、voltarget_only:log 未支配（依雙支配規則不翻），**但差異在雜訊內**——所有格追蹤 σ* 僅 0.28-0.41%（追蹤誤差軸幾乎平），支配由端點 <1% turnover 差決定；正式定案待寬網格＋成本 bps＋bootstrap 顯著性（列為後續）。衍生兩張 code-review 後續卡：去重 `vol_target_ac.realized_annual_vol`↔`metrics.annualized_vol`、修既有環境性 flaky e2e `test_ac2_submit_real_config_runs_to_done`（引擎子行程於沙箱啟動即消失，base commit 同樣重現、與本次無關）。設計/計畫見 `docs/superpowers/specs/2026-08-13-exposure-band-log-mode-design.md`＋`plans/2026-08-13-exposure-band-log-mode.md`。**main 領先 origin 未推。**
- 2026-08-14：**AC5 正式定案——維持 default=absolute（log 未打敗簡單基線）**。以 `experiments/ac5_exposure_band.py` 重跑：寬網格（absolute{0.05-0.22}/log{0.10-0.40}）、`voltarget_only`+`full`、GARCH 全歷史各格留 nav，輸出 turnover / σ* 追蹤誤差 / **成本拖累(bps/年)** / Sharpe 前沿；每策略取 turnover 最接近的一對做 stationary block bootstrap 配對 Sharpe 差 CI（§6.3，seed 走 config 守 INV-6）。**結果**：`voltarget_only` 匹配對 abs(0.22)/log(0.30) Sharpe 差(log−abs) −0.0014、CI [−0.035,+0.043]；`full` 匹配對 abs(0.22)/log(0.30) 差 +0.0039、CI [−0.015,+0.023]——**兩者 CI 皆含 0、點估計 ~±0.004、成本差 <1 bps/年、追蹤誤差兩 mode 皆 <0.5%**。**先前 9 格 smoke 的「full:log 支配」證實為端點雜訊假象**（寬網格+bootstrap 反駁）。與 DCC/ERC/garch_own 一致：加的複雜度未自證其值 → **簡單者續為預設**，log 帶留已驗證 config 選項。**附帶觀察（另案、未動）**：與 mode 無關，帶寬 0.10→0.16~0.22 在兩 mode 內皆單調降 turnover 與成本（full 省 ~10 bps/年）而 Sharpe/追蹤無明顯代價——「帶寬度」調校題，若在意成本值得另開一題（同樣需 bootstrap 確認）。分析腳本收進 `experiments/ac5_exposure_band.py`（`--no-ff` 併回 main）。
- 2026-08-14：**帶寬度調校探索（輕流程，結論維持 exposure_band=0.10）**。以 scratchpad 腳本 + ewma 快配置掃 absolute 帶寬 0.06-0.28（voltarget_only+full，重用 `ac5_exposure_band` 積木、不寫 runs/）。**發現**：帶變寬 → turnover/成本單調下降（0.10→0.28 省 ~7-10 bps/年），但 (1) Sharpe 平在雜訊內（full 0.61-0.64 無趨勢）、(2) **full 的 MaxDD 隨帶變寬變差 −15.5%→−16.3%（~0.75pp，符合「帶寬→去風險慢→回撤深」預期）**、(3) 追蹤誤差升至 0.4-0.5% 仍在容差內。**結論：邊際優化（省 ~5 bps 換 ~0.75pp 較深回撤），不值得翻預設，維持 0.10**；若日後成本成痛點，甜蜜點約 0.16，需 GARCH+bootstrap 確認 MaxDD 差顯著性才動。此為研究探索（見記憶 research-experiment-fast-loop），非生產變更、無 code 改動。
- 2026-08-15：**新策略 `full_gjr`（GJR-GARCH 波動引擎）完成——GJR 未打敗標準 GARCH，維持 `full` 為主策略**（分支 `feature/full-gjr-strategy`）。動機：`full` 的 σ̂ 用對稱 GARCH(1,1)-t，無法捕捉槓桿效應（負向衝擊比同幅正向更放大波動）。新增 GJR-GARCH（arch `o=1`，非對稱項 γ）為 `full_gjr` 策略。**架構關鍵**：σ̂ 的兩個下游用途（Step 2 σ̂_i→inverse-vol 權重、Step 3 σ̂_p→曝險 E(t)）吃同一份 forecaster 輸出（Step 3 共變異數對角線＝Step 2 個別 σ̂），故只需**單一抽換點**（forecaster spec→`gjr_garch`）即同時覆蓋兩點。實作：`GjrGarchArch(GarchArch)` 子類（`_o=1`，`_build_arch_model`/濾波函式加 `o` 參數、單一模型建構來源不分歧）、**平穩條件納入 γ**（`α+β+0.5γ<1`，0.5 提為 `_GJR_NEG_INDICATOR_EXPECTATION`，純 GARCH γ=0 行為不變，INV-4）、`fit_volatility`/`VolForecaster` 認 `gjr_garch`（快取 `arch_o`、`last_params` 依向量順序 `[μ,ω,α,γ,β,ν]` 取 γ、GJR fit 失敗一樣 fallback EWMA）、`FullGjr(Full)` **內部強制** GJR（不依 config.vol_model，比照 `full_erc` 策略即身份，使 full vs full_gjr 可同 suite 對比）、`gjr_garch` **不進 config VolModel**（僅內部 spec，YAGNI）、前端 GARCH 頁 persistence 納入 0.5γ + 參數軌跡加 γ 系列（presentation 不 import 引擎，0.5 必要本地複寫並註明同步）。以 subagent-driven TDD 執行 6 個實作 task，每項經 spec + code-quality 兩段式 review，**抓修數項**（`arch_params` docstring 漏 GJR 6 元素向量、base 兩處舊平穩條件 docstring、`last_params` 位置解碼缺值交叉驗證測試→補「位置 vs 具名」一致斷言防未來 arch 順序變動靜默分歧、前端 0.5 未具名+chart-cols 分支未測→抽 `_param_chart_cols` 純函式補測）。全套測試綠（含 INV-1~6、既有 GARCH 回歸、前端 92）。**AC 誠實結論**（消融 `canonical_dcc`、GARCH+DCC 全歷史 2005-、20 檔，`scratchpad/gjr_ablation.py`）：**GJR 確實估到有意義的非對稱項**（γ 中位數 0.138、85.9% 為正，非退化到 EWMA），但**組合層級效果邊際且不明確有利**——full vs full_gjr 年化報酬同為 7.22%、Sharpe 0.591 vs 0.589（−0.002）、Sortino +0.005、**MaxDD −14.9% vs −15.4%（略差）**、Calmar 0.486 vs 0.470（−0.016）、**年化換手 6.15→6.60（+0.44，GJR 更貴）**、平均曝險 +0.9pp。曝險/σ̂_p 選擇日對齊 n=1229 高度相關（corr 0.96/0.97、個別日 max|Δ|曝險 0.21 但整體互抵）。**與 DCC/ERC/garch_own 一致：GJR 沒賺回多出的換手，簡單者（標準 GARCH）續為預設**，`full_gjr` 註冊為已驗證的研究/候選策略。設計/計畫見 `docs/superpowers/specs/2026-08-15-full-gjr-strategy-design.md` + `plans/2026-08-15-full-gjr-strategy.md`。
- 2026-08-16：**full_gjr 後續 horizon 敏感度——縮短 forecast_horizon 不救 GJR 反更糟；定位「反應式而非預測式」（研究輕流程，無 code 改動）**。承上條消融「GJR 有意義 γ 但組合無感」，深挖不顯著來源（腳本 `scratchpad/gjr_per_asset.py`/`gjr_horizon_sens.py`/`gjr_h5_backtest.py`/`gjr_covid_example.py`，未進 repo）。**逐資產 γ**：槓桿效應真實且挑資產——股票 γ 0.13–0.23（QQQ/XLK/SPY/XLF 高）、債券/黃金 ≈0 甚至負（GLD −0.06、TLT/IEF ~0.02）；動量未系統性避開高 γ 資產（內生 γ×選中頻率相關僅 −0.17、20 檔全輪到過），**排除「資產」與「動量選股」兩個嫌疑**。**σ̂ 層**：被選中資產 relΔσ̂（GJR−GARCH）帶號均值僅 −1.4%（|6%|），γ 的近端放大被 H=21 年化平均攤平。**H=5 vs H=21 滾動敏感度（1302 樣本）證實稀釋**：最近一日負報酬桶，H=5 帶號 relΔσ̂ **+1.6%**（高 γ 股票 +2.5%）vs H=21 **+0.01%**（0.3%）——γ 的方向性反應完全活在近端、H=21 積分掉。**但組合層級 2×2 實測（{full,full_gjr}×{H21,H5}）推翻「縮短 horizon 可救」直覺**：H=5 下 full_gjr 每項指標都**更差**（Sharpe −0.022、Sortino −0.028、MaxDD 更深 −0.012、Calmar −0.048、換手 +0.51；full_gjr 自身 H5−H21 亦全面惡化）。**機制反轉**：GJR 是「跌後才拉高 σ̂」的**反應式**訊號，在 vol-target overlay 裡＝晚砍＋來回抽；H=21 不是藏住好處，而是**無意間把淨有害的反應式訊號平均掉**——縮短 horizon 等於把它放出來，故更糟。**2020/3 COVID 舉例（H=5、XLK γ≈0.3）**：崩盤前平靜期 GJR σ̂ 較低→full_gjr **更高曝險進場**（2/11 E 0.88 vs full 0.80，多吃第一段跌）；3/12 XLK σ̂ GJR **1.154 vs GARCH 0.668**（近 2×）砍在谷底；4/2–4/9 GJR σ̂ 快速崩落→re-risk 較快（E 0.34 vs 0.28）。此次 V 型反彈使該段近打平（full_gjr 谷底 0.874 略淺、末日 0.913 略高），**淨損來自非 V 型（緩跌/來回抽）情境的「崩前過曝＋崩後晚砍」兩頭吃虧**。**結論定案**：槓桿效應是**描述性真實但非有用預測**；regime 去風險已由動量+絕對動量層（預測式/趨勢式）承擔，GJR 只加反應式雜訊。`full`（garch_arch, H=21）續為預設；**full_gjr 為有價值的負面結果**（不只「無效」，而是「用短 horizon 想救會更糟，因其本質反應式」），留研究/候選。與 DCC/ERC/garch_own/log-band 一致：加的複雜度未自證其值。
- 2026-08-21：**穩健性統計嚴謹性補洞完成並兩批 `--no-ff` 併回 main**——補上規格 §6.5 定義的多重檢定校正層。**批一（`feature/significance-testing`）**：`backtest/significance.py`（PSR / expected_max_sharpe / DSR、PBO via CSCV、蒙地卡羅區塊置換）、`experiments/significance_report.py` + CLI（讀消融 run 產 `significance.json`）、`ablation.py` 落 `cell_returns.parquet`、config `stats.{significance_strategy,psr_benchmark_sr,pbo_n_splits,mc_permutations}`。以 subagent-driven TDD 執行 6 task，兩段式 review 抓修一個**會憑空製造顯著性的真 bug**（`permutation_test_paired` 在 `observed=NaN` 時回 p≈0.005，改回 NaN）＋目標策略存在性 guard＋計畫測試單 seed 不穩健（改多次抽樣取平均，PBO 虛無期望=0.5）。**批二（`feature/significance-ladder`）**：把結論從「full-vs-all 混比」收斂為**巢狀消融階梯分指標**——`metric_max_drawdown` 陣列版、config `stats.ablation_ladder`（預設 `[bh_spy,mom_only,mom_ivol,full]`，經核實 §6.3 策略組成；`voltarget_only` 非線性巢狀故排除）、`significance_report._ladder_analysis` 對相鄰步 × {Sharpe,Calmar,MaxDD} 產 bootstrap 配對 CI + 置換 p-value。以 subagent-driven TDD 執行 4 task，過程 implementer 自行抓修一個真回歸（既有測試未覆寫 ladder → 預設階梯策略缺席 ValueError）。兩批合併後不變量 37/37 + 相關子集全綠、ruff 全樹乾淨。判準：**bootstrap CI 排除 0 且 置換 p<0.05**（兩套不同假設一致）才算穩健顯著。**誠實科學結論（快照 `2026-07-16_20ed09`，預設 10 消融格 N）**：
  - **唯一穩健顯著的層貢獻＝步 3「組合波動目標」對 MaxDD 的改善**：obs +0.108、CI=[+0.032,+0.171] **排除 0**、置換 p=0.010。
  - **步 1「動量選股」對 MaxDD 有邊際訊號**（置換 p=0.033）**但 bootstrap CI 含 0**（[−0.070,+0.315]）→ 兩套檢定不一致，不算站得住；其 Sharpe/Calmar 皆不顯著（p 0.81/0.33）。
  - **步 2「inverse-vol 定倉」在三個指標上全無可測貢獻**（obs≈0，Sharpe/Calmar/MaxDD p=0.94/1.0/0.19）——**最該被質疑的層**（§6.3「移除無貢獻的層」的直接候選；與 5b「ERC/inverse-vol 權重方案無顯著差異」一致）。
  - **無任何一層顯著改善 Sharpe**（全 p≥0.81）；`full` 的價值本質是**回撤/風險管理**，非提升報酬。
  - DSR=0.991（但**弱校正**：N=10、格間 Sharpe 離散度極低 `sr_variance`≈8e-6，高值主要來自 full 每期 Sharpe>0 而非通過強多重檢定）；PBO≈0.455（消融格選擇無可泛化優勢，因各格近乎同質）。
  - **限制/後續**：單一快照、DSR 的 N 僅涵蓋單次消融、置換未分子期間。與 Phase 4c/5a/5b 一貫——複雜度在本樣本期未自證其值，唯一站得住的是 vol-target 層的回撤控制。**待辦：inverse-vol 定倉層待正式複核（是否移除）；顯著性分子期間（2005-09/2010-19/2020-）與 walk-forward OOS+embargo 為建議下一步**。設計/計畫見 `docs/superpowers/specs/2026-08-16-significance-testing-design.md`、`2026-08-21-significance-ladder-design.md` 及對應 plans。
